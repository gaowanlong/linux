...
struct virtio_scsi_target_state {
	...
	/* The number of outstanding requests - used to detect idle target */
	atomic_t reqs;
...
static int __devinit virtscsi_probe(struct virtio_device *vdev)
{
	...
	u32 cmd_per_lun;
	u32 num_queues;
	struct scsi_host_template *shostt;
	...
	/* Multiqueue is not benificial with a single target. */
	if (num_targets == 1)
		num_queues = 1;

	if (num_queues == 1)
		shostt = &virtscsi_host_template_single;
	else
		shostt = &virtscsi_host_template_multi;

	shost = scsi_host_alloc(shostt,
